...
const callFlutter = (name, data) => {
  // Prefer InAppWebView bridge (Anx native), fallback to webview_flutter channel.
  if (window.flutter_inappwebview?.callHandler) {
    window.flutter_inappwebview.callHandler(name, data)
    return
  }
  if (window.YaruBridge?.postMessage) {
    window.YaruBridge.postMessage(JSON.stringify({ name, data }))
    return
  }
  // Last-resort fallback for debugging environments.
  console.log('[YaruBridge]', name, data)
}
...
// Runtime params can come from URL (legacy) or host bootstrap bridge.
const _safeParseJson = (value, fallback = null) => {
...
const _applyRuntimeParams = (params) => {
  importing = !!params.importing
  url = params.url || ''
  initialCfi = params.initialCfi || ''
  style = params.style || {}
  readingRules = params.readingRules || {}
}

const _startReader = (params) => {
  _applyRuntimeParams(params)
  if (!url) return
  fetch(url)
    .then(res => res.blob())
...
window.__yaruOpenFromHost = (params) => {
  _startReader(params || {})
}

const urlParams = new URLSearchParams(window.location.search)
const paramsFromUrl = {
  importing: _safeParseJson(urlParams.get('importing'), false),
  url: _safeParseJson(urlParams.get('url'), ''),
  initialCfi: _safeParseJson(urlParams.get('initialCfi'), ''),
  style: _safeParseJson(urlParams.get('style'), {}),
  readingRules: _safeParseJson(urlParams.get('readingRules'), {}),
}

if (paramsFromUrl.url) {
  _startReader(paramsFromUrl)
} else if (window.__yaruReaderParams) {
  _startReader(window.__yaruReaderParams)
}
